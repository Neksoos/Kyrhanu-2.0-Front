...
async def _load_loot_items() -> Dict[str, Any]:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            SELECT
                ...
                atk, defense, hp, mp, level_req, sell_price, stats,
                is_archived, is_lootable
            FROM items
    ...
    for r in rows:
        # ✅ тепер керуємо дропом через колонку
        if not bool(r.get("is_lootable", False)):
            continue
        ...
        # ── EQUIP (тільки lootable=true, вже відфільтровано вище)
        if (
            r["category"] == "equip"
        ...
        # ── TRASH/JUNK (також lootable=true, вже відфільтровано вище)
...
async def get_loot_for_mob(mob_code: str, *, area_key: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    mob_code наразі не використовується (можна додати дроп-таблицю по мобах пізніше).
    """
