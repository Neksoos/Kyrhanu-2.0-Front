...
from services.quest_drops import roll_quest_drops  # ✅ NEW
...
def _mob_area_key(mob: Mob) -> Optional[str]:
    """
    Повертає area key для дропа.
    Підтримує різні варіанти полів, щоб не ламало модель.
    """
    for attr in ("area_key", "area", "areaCode", "area_code"):
...
async def reward_items_new(tg_id: int, mob: Mob) -> List[str]:
    area_key = _mob_area_key(mob)

    # 1) звичайний лут (equip/trash/junk)
    try:
        loot_items = await get_loot_for_mob(mob.code, area_key=area_key)
        logger.info("loot: mob={} lvl={} area={} result={}", mob.code, mob.level, area_key, loot_items)
    except Exception:
        logger.exception("loot: get_loot_for_mob FAILED tg_id={} mob_code={}", tg_id, mob.code)
        loot_items = []

    # 2) квестові дропи (misc/consumable з stats.quest_item=true)
    try:
        ...
        if quest_items:
            logger.info("quest_loot: mob={} lvl={} area={} items={}", mob.code, mob.level, area_key, quest_items)
    except Exception:
        logger.exception("quest_loot: roll_quest_drops FAILED tg_id={} mob_code={}", tg_id, mob.code)
        quest_items = []

    all_items = []
    if loot_items:
        all_items.extend(loot_items)
    if quest_items:
        all_items.extend(quest_items)

    if not all_items:
        return []

    try:
        return await distribute_drops(tg_id, all_items)
    except Exception:
        logger.exception("loot: distribute_drops FAILED tg_id={} loot={}", tg_id, all_items)
        return []
...
async def reward_for_win(tg_id: int, mob: Mob, battle_id: Optional[int] = None) -> List[str]:
    ...
    # ----------------------------
    # Items loot (звичайний + квестовий)
    # ----------------------------
    try:
