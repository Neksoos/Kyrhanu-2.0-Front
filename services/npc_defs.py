...
def _default_npc_image_url(npc_key: str) -> str:
    # Файли лежать у /public/npc/{npc_key}.png (Next.js)
    return f"/npc/{npc_key}.png"
...
class NpcDef:
    ...
    # ✅ НОВЕ: шлях до портрета NPC (можна не задавати — тоді /npc/{key}.png)
    image_url: Optional[str] = None

    def to_dict(self) -> Dict:
        img = self.image_url or _default_npc_image_url(self.key)
        return {
            "key": self.key,
            "name": self.name,
            "region": self.region,
...
OKSANA = NpcDef(
    ...
    weight=2,
    image_url=None,  # можна опустити; залишив для явності
)
...
REGISTRY_VERSION = 2  # ✅ інкрементнули, бо додали image_url
