...
async def ratings_nightwatch(u: dict = Depends(get_tg_user)) -> NightResp:
    tg_id = int(u["id"])
    ...
    def coerce(row: dict) -> dict:
        r = dict(row or {})
        # якщо сервіс вже віддає tg_id — ок
        if "tg_id" not in r:
            # інколи в сервісах буває ключ id
            if "id" in r:
                try:
                    r["tg_id"] = int(r["id"])
                except Exception:
    ...
    top_rows = [NightWatchRow(**coerce(r)) for r in (top_rows_raw or [])]
    you_row = NightWatchRow(**coerce(you_raw)) if you_raw else None

    return NightResp(top=top_rows, you=you_row)


@router.get("/sacrifice", response_model=SacrificeResp)
async def ratings_sacrifice(u: dict = Depends(get_tg_user)) -> SacrificeResp:
    tg_id = int(u["id"])
...
async def ratings_perun(
    ...
    scope: str = Query("week", regex="^(day|week|month|all)$"),
) -> PerunResp:
    tg_id = int(u["id"])
