...
try:
    ...
    async def start_encounter(_uid: int, _npc_key: str) -> None:  # type: ignore
        return None

    async def accept_quest(_uid: int, _npc_key: str) -> str:  # type: ignore
        return "Прийнято."

    async def decline_quest(_uid: int, _npc_key: str) -> str:  # type: ignore
        return "Відмовлено."

    async def extra_line(_uid: int, _npc_key: str) -> Optional[str]:  # type: ignore
        return "— Та що там ще казати…"
...
async def _get_npc_safe(npc_key: str):
    """
    _get_npc може бути sync або async.
...
# ────────────────────────────────────────────────────────────────────
# FastAPI router  ✅ ВАЖЛИВО: оголосити ДО декораторів
# ────────────────────────────────────────────────────────────────────
...
async def npc_encounter(npc_key: str, req: EncounterRequest):
    ...
    # ✅ фіксуємо encounter (вибір активного квесту під гравця)
    await start_encounter(req.tg_id, npc_key)
...
async def npc_accept(npc_key: str, req: ResultRequest):
    ...
    msg = await accept_quest(req.tg_id, npc_key)
...
async def npc_decline(npc_key: str, req: ResultRequest):
    ...
    msg = await decline_quest(req.tg_id, npc_key)
...
async def npc_more(npc_key: str, req: TipRequest):
    ...
    tip = await extra_line(req.tg_id, npc_key)
