...
quest details, advancing through quest stages, and listing a player's
...
# ✅ ABSOLUTE imports
from services.quest_engine import (
    start_quest,
    get_player_stage,
    advance,
    list_player_quests,
...
@router.get("/player/{tg_id}")
async def player_quests(tg_id: int):
    """
    Return the list of active and completed quests for the given player.

    The result contains two keys: ``active`` and ``completed``, each of
    which is a list of quest progress records as defined by
    ``services.quest_engine.list_player_quests``.
    """
    return list_player_quests(tg_id)
