...
# NOTE: імпорт модулем менш крихкий, ніж `from ... import ...`.
# Якщо десь з’явиться циклічний імпорт, сервер не впаде на старті.
from services import gathering_loot
...
async def complete_gathering_task(tg_id: int) -> tuple[GatheringTask, List[Dict[str, Any]]]:
    ...
    async with pool.acquire() as conn:
        ...
        drops_raw = await gathering_loot.roll_gathering_loot(
            tg_id=tg_id,
