  ...
  async function loadAll() {
    try {
      ...
      const [recipesRaw, me, matsRaw, itemsRaw] = await Promise.all([
        getJSON<RecipesResponse>("/api/craft/recipes?profession=weaver"),
      ...
      setProf(me.professions.find((p) => p.profession.code === "weaver") ?? null);
      setError(null);
    } catch (e: any) {
      setError(e?.message || "Не вдалося завантажити ткача");
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="min-h-screen bg-slate-950 text-white p-4">
      <div className="mx-auto max-w-4xl space-y-4">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <h1 className="text-xl font-semibold">🧵 Ткач</h1>
          <p className="text-sm text-slate-300">Створюй тканину, мантії, накидки та легку броню.</p>
          <div className="mt-2 text-xs text-slate-300">Рівень {level} · XP {xp}/{next}</div>
          <div className="mt-1 h-2 rounded bg-slate-800"><div className="h-2 rounded bg-fuchsia-400" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} /></div>
        </section>
              ...
              return (
                <div key={r.code} className="rounded-2xl border border-white/10 bg-black/30 p-4">
                  <div className="flex justify-between gap-3">
                    ...
                    <button disabled={disabled} onClick={() => craft(r)} className="rounded-xl bg-fuchsia-500 px-3 py-2 text-sm font-semibold text-black disabled:opacity-40">
